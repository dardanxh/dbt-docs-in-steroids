...
import type { ColumnLineage, ColumnUsage, GraphResponse, ModelError, NodeDetail } from "@/types";
...
export const lineageKeys = {
  ...
  nodeErrors: (projectId: string, nodeId: string) => ["node-errors", projectId, nodeId] as const,
...
export function useNodeErrors(projectId: string | null, nodeId: string | null) {
  return useQuery({
    queryKey: lineageKeys.nodeErrors(projectId ?? "", nodeId ?? ""),
    queryFn: () =>
      apiGet<ModelError[]>(`/projects/${projectId}/nodes/${encodeURIComponent(nodeId ?? "")}/errors`),
    enabled: !!projectId && !!nodeId,
  });
}
